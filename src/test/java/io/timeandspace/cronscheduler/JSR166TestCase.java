...
//                ThreadPoolExecutorTest.suite()
    ...
    PoolCleaner cleaner(CronScheduler scheduler) {
        return new PoolCleaner(scheduler.asScheduledExecutorService());
    }
     ...
     * startMillis, which must have been previously returned from a
     * call to {@link System#currentTimeMillis()}.
     */
    static long millisElapsedSince(long startMillis) {
    ...
    void assertTaskSubmissionsAreRejected(CronScheduler p) {
        ...
        final int savedQueueSize = p.getQueueInternal().size();
...
//                assertSame(r, recorder.r);
            ...
            recorder.reset();
            assertFalse(ses.submit(r).isDone());
            if (stock) assertTrue(!((FutureTask) recorder.r).isDone());
            assertSame(p, recorder.p);

            recorder.reset();
            assertFalse(ses.submit(r, Boolean.TRUE).isDone());
            if (stock) assertTrue(!((FutureTask) recorder.r).isDone());
            assertSame(p, recorder.p);

            recorder.reset();
            assertFalse(ses.submit(c).isDone());
            if (stock) assertTrue(!((FutureTask) recorder.r).isDone());
            assertSame(p, recorder.p);
        ...
        assertEquals(savedQueueSize, p.getQueueInternal().size());
    }
